...
class ZipService
  ...
  def load_css_starter(css_starter, hash)
    if css_starter == 'normalize'
      hash['css/normalize.css'] = File.read('./public/stylesheets/css/normalize.css')
    elsif css_starter == 'reset'
      hash['css/reset.css'] = File.read('./public/stylesheets/css/reset.css')
    end
  end
  
  def load_prefix(prefix, hash)
    if prefix != '' and !prefix.nil?
      hash['js/prefixfree.min.js'] = File.read('./public/js/libs/prefixfree.min.js')
    end
  end
  
  def load_modernizr(js_modernizr, hash)
    if js_modernizr != '' and !js_modernizr.nil?
      hash['js/modernizr.js'] = File.read('./public/js/libs/modernizr.js')
    end
  end
